...
| https://github.com/eclipse/microprofile-lra/tree/master/api/src/main/java/org/eclipse/microprofile/lra/annotation/TimeLimit.java#L30[@TimeLimit]
  | Limits the time that an LRA or a compensation remains valid for.
|===
...
`LRA.Type.REQUIRES_NEW`::
    ...
    was invoked and the `LRA#delayClose()` attribute is set to true
...
`LRA#delayClose()` attribute which will leave the new LRA active
...
it in a different method using the `delayClose` and `terminal` attributes.
...
The `delayClose = true` attribute on the bookTrip method forces the LRA to continue running when
...
that start LRAs with the `@TimeLimit` annotation footnote:[issue #34 will change
this to be an attribute of both the LRA, Compensate and Complete annotations].
For example, to indicate that an LRA should automatically cancel after 100 milliseconds:
...
`@TimeLimit` annotation to the `@Compensate` method. An example of how a participant
