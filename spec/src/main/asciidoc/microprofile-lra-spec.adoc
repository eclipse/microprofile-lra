...
This specification has 1 configuration parameter which is retrieved using the MicroProfile Config principals.

     mp.lra.LRA.value=SUPPORTS

The allowed values for the configuration parameter are `SUPPORTS`, the default value, and `NOT_SUPPORTED`.
When an invalid value is defined, the implementation MUST prohibit the successful startup of the application
(e.g. through a runtime exception).
...
the system behaves as if there is an annotation present with the `type()` value taken from the configuration parameter.

Note however that these values, which correspond to the enum values of `org.eclipse.microprofile.lra.annotation.ws.rs.LRA.Type`
only define how the LRA header information is propagated when an outgoing request is performed from such a JAX-RS resource.
...
request in the case the configuration value `SUPPORTS` is specified. All values for the headers defined in `org.eclipse.microprofile.lra.annotation.ws.rs.LRA.LRA_HTTP_CONTEXT_HEADER`
...
If an LRA is propagated to a resource that is not annotated with `@LRA` then the implementation behaves
as if there is an annotation present on the resource with the `type()` defined by the configuration parameter
`mp.lra.LRA.value`. From a functional point of view, there is no difference notable except for the different
default for `end()` which is false when there is no `@LRA` defined.
