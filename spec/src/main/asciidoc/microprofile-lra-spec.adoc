...
In case `anotherService` is called within the same Long Running Action, this is the LRAid is in the header for
the call at the endpoint, at 10h27, the timeout related to this call is 10h28. Since there are 2 timeouts available
for the Long Running Action, the earliest one is chosen.
The LRA will be eligible for automatic cancellation from 10h28 onwards although the time out was initially 1 hour,
