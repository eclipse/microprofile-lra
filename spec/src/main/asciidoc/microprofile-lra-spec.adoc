...
made during the execution of the method will still carry the original headers
only when the configuration parameter `mp.lra.header.propagation` is set to `true`.

By default, this configuration parameter `mp.lra.header.propagation` has the value `false` so that
only an `active context` is propagated on outgoing JAX-RS requests.

This configuration parameter is retrieved using the MicroProfile Config specification rules.
