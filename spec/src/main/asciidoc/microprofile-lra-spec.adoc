...
call. Just as if the Long Running Action would be closed due to an error.

When a participant joins an existing transaction using the `@LRA(value = LRA.Type.MANDATORY)` annotation, the timeout of
...
An example to illustrate this, suppose you have the following endpoint definitions

[source,java]
----
  @LRA(timeLimit = 1, timeUnit = ChronoUnit.HOURS, end = false)
  @Path("/someService")
  @GET
  public Response someService(...) {...}

  @LRA(value = LRA.Type.MANDATORY, timeLimit = 1, timeUnit = ChronoUnit.MINUTES, end = false)
  @Path("/anotherService")
  @GET
...
Suppose you have a call to the `someService` endpoint at 10h25, the time out is at that moment calculated at 11h25.
Suppose that `anotherService` is called for the same Long Running Action, this is the LRAid is in the header for
the call at the endpoint, at 10h27. The timeout related to this call is 10h28. Since these 2 calls share the same
Long Running action, the LRA will be eligible for automatic cancellation from 10h28 on although the time out was initially 1 hour,
it can be cancelled already after 3 minutes.
