...
public class TckLRATypeTests extends TckTestBase {
    ...
    /**
     * Perform a JAX-RS resource request and check the resulting status and whether or not it ran with the correct LRA
     * context.
     *
     * @param rootPath
     *            the root path of the JAX-RS resource to invoke
     * @param path
     *            the resource path of the JAX-RS method to invoke
     * @param startLRA
     *            indicates whether or not an active context should be present on the request
     * @param expectedStatus
     *            the expected JAX-RS status code in the response
     * @param lraCheckType
     *            the type of check to perform on the request context and the context specified by the {@link LRA.Type}
     *            annotation
     * @param methodLRAShouldBeActive
     *            if true the LRA started by the invoked method should still be active after the resource invocation
     *            completes
     */
    private String resourceRequest(String rootPath, String path, boolean startLRA, int expectedStatus,
            MethodLRACheck lraCheckType,
        ...
        try {
            ...
            switch (lraCheckType) {
                case NOT_PRESENT :
                    assertEquals(
                            testName.getMethodName() + ": Resource method should not have run with an LRA: "
                                    + methodLraId,
                ...
                case NOT_EQUALS :
                    assertNotEquals(
                            testName.getMethodName() + ": Resource method should not have run with the incoming LRA",
                            incomingLraId, methodLraId);
                    break;
                default :
                    break;
            ...
            if (methodLRAShouldBeActive) {
                ...
                assertNotEquals(
                        testName.getMethodName() + ": Resource method should not have run with an LRA: " + methodLraId,
                        0, methodLraId.length());
